...
class SignInDetailElement extends \ContentElement
{
    /**
     * @var string
     */
    protected $strTemplate = 'cte_sign_in_detail';
    ...
    private function generateBackend()
    {
        ...
        $this->Template->wildcard   = "### Sign-In Detailansicht ###";
    }
    ...
    private function generateFrontend()
    {
        ...
        if($_GET['item']){
            ...
            if($eventModel instanceof \Contao\CalendarEventsModel){
                $this->Template->pins = self::getSignedInUserList($eventModel->id);
            }
        }
    }

    public static function getSignedInUserList($eventId)
    {
        $return = array();
        ...
        if(is_array($participants) && count($participants) > 0){
            foreach ($participants as $p){
                if($p['subscribe_data']['status'] === 'signIn'){
