...
use Home\LibrareeBundle\Resources\contao\models as LibrareeModel;

class EventListWithSubscription extends \ModuleEventlist
{
    /**
     * Template
     * @var string
     */
    protected $strTemplate = 'mod_subscribeeeventlist';
     ...
     * Generate the module
     *
     * Dies ist die Compile Funktione von ModuleEventlist; Da die Darstellung der Events in dieser Funktion gerendert wird,
     * muss die gesamte Funktion hier nochmals aufgeführt werden und unserer Part integriert werden!!!
     */
    protected function compile()
    {
        ...
        for ($i=$offset; $i<$limit; $i++)
        {
            ...
            $objTemplate->event_id = $event['id'];
            $objTemplate->loadPage = null;
            $objTemplate->eventInPast = false;
            $objTemplate->formularId = $objTemplate->formularId = $this->subscribee_formular;
            $objTemplate->freePlaces = Model\Subscribee::getFreePlacesByEventId($event['id']);
            $objTemplate->maxPlaces = Model\Subscribee::getMaxPlacesByEventId($event['id']);
            $objTemplate->resPlaces = Model\Subscribee::getReservedPlacesByEventId($event['id']);
            $start = ($event['startTime']) ? $event['startTime'] : $event['startDate'];
            if ($start < time()) {
                $objTemplate->formularId = 0;
                $objTemplate->eventInPast = true;
            }

            $options['filter']['tl_lib_pin.pid'] = ' = 2';
            $taxonomies = LibrareeModel\Pin::findByFilters($options);
            $tax = array();
            foreach ($taxonomies as $taxonomy){
                $tax[$taxonomy['id']] = $taxonomy;
            }
            $objTemplate->taxonomies = $tax;
        ...
        #-- set $taxonomies for mod_eventlist_isotope
        $this->Template->taxonomies = $tax;
